...
package body Testgen_Translator is
  ...
  procedure Put_Descriptions_Of_Output_Modes is
  begin
     Put_Line ("   off       No output.");
     Put_Line ("   numbers   Only test case numbers");
     Put_Line ("   locations Numbers, line-numbers (in test script) " &
               "and name of test script");
     Put_Line ("   titles    Numbers and titles");
     Put_Line ("   full      Everything, including a short explanation.");
  end Put_Descriptions_Of_Output_Modes;
  ...
  procedure Translate (Input_File   : in Text_IO.File_Type;  ------------------
                       ...
                       Long_Trailer : in Boolean := True) is
    ...
    Line_Number : Natural := 0;  -- line number of current test case
    ...
    procedure Put_Location(Condition: in Boolean := True) is
    begin
       if Condition then
          Put("Script name:'");
          Put(Script_Name);
    ...
    begin
      ...
      case Fail_Output is
        when Off     =>
           null;
        when Numbers | Locations =>
           Put_Indent; Put ("Put_Line (""("); Put (Test_Case_Number, 0);
           Put(") ");
           Put_Location (Fail_Output = Locations);
           Put_Line(" FAIL."");");
        when Titles =>
    ...
    begin
      ...
      case Pass_Output is
        ...
        when Numbers | Locations =>
           Put_Indent; Put ("Put_Line (""("); Put (Test_Case_Number, 0);
           Put (") ");
           Put_Location (Pass_Output = Locations);
           Put_Line (" pass."");");
        when Titles =>
          if not (    Pass_Output in Titles..Full
                  and Fail_Output in Titles..Full) then
    ...
    begin
      ...
      Put_Indented ("  Driver_Internals.Unexpected_Error := True;");
      Put_Indented ("  Put_Line (""ERROR: exception "" & " &
                    "Ada.Exceptions.Exception_Name (E) & "" raised in " &
                    Part_Name & " of test case" &
                    Positive'Image (Test_Case_Number) & "."");" );
    ...
    begin
      ...
      Put_Indented ("then");
      Indent := Indent + Indent_Step;
        -- "then" in a new line, to make sure it is not put behind a comment
      Put_Pass (Test_Case, "path `" & Path &
                "' was taken and predicate is TRUE");
      Indent := Indent - Indent_Step;
      Put_Indented ("else");
      Indent := Indent + Indent_Step;
      Put_Fail (Test_Case, """path `" & Path &
                "' was taken, but predicate is FALSE""");
      Indent := Indent - Indent_Step;
      Put_Indented ("end if;");
    ...
    begin
      ...
      Indent := Indent + Indent_Step;
      Put_Fail (Test_Case,
                """path `"" & Driver_Internals.Taken_Path & ""' when `" &
                Path_Names (Expected_Paths) & "' was expected""");
      Indent := Indent - Indent_Step;
      Put_Indented ("end if;");
    ...
    begin
      Put_Line(Magic_String_1);
      Put_Line(Magic_String_2);
      New_Line;
    ...
    begin
      ...
      if Long_Trailer then
         Put_Line ("  New_Line;");
      end if;
      Put_Line ("  Put (""Script name '" & Script_Name & "'" & """);");
    ...
    begin
       Line_Number := Test_Script_Scanner.Current_Line;
  ...
  begin  -- Translate
    ...
    Test_Script_Scanner.Next_Token;
    Translate_General_Part;

    loop
      case Test_Script_Scanner.Current_Token is
        when Test_Script_Scanner.End_Of_Input =>
          exit;
        when Test_Script_Scanner.Key_Begin_Test_Case =>
          Translate_Test_Case;
          Test_Case_Number := Test_Case_Number + 1;
        when Test_Script_SCanner.Key_Code =>
          Translate_Code;
        when Test_Script_Scanner.Error =>
          raise Syntax_Error;
        when others =>
          raise Syntax_Error;
      end case;
